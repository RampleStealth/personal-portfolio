...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><SiteHeader /><main id="main"><article>
    <header className="mx-auto max-w-7xl px-5 pb-16 pt-12 lg:px-8 lg:pb-24 lg:pt-20">
      <Link className="inline-flex items-center gap-2 text-sm text-ink/60 transition hover:text-ink" href="/#work"><ArrowLeft size={16} /> Selected work</Link>
      <div className="mt-14 grid gap-10 lg:grid-cols-12 lg:items-end">
        <div className="lg:col-span-7"><p className="eyebrow">{project.number} · {project.category}</p><h1 className="mt-4 font-display text-5xl tracking-[-.045em] sm:text-7xl">{project.name}</h1><p className="mt-7 max-w-2xl text-xl leading-relaxed text-ink/65">{project.summary}</p></div>
        <div className="border-l-2 border-moss pl-4 lg:col-span-3 lg:col-start-10"><p className="text-xs uppercase tracking-[.14em] text-ink/45">Status</p><p className="mt-2 text-lg font-medium text-moss">{project.status}</p></div>
      </div>
    </header>
    ...
    <section className="mx-auto max-w-7xl px-5 pb-20 lg:px-8 lg:pb-32"><div className="grid gap-16 lg:grid-cols-12"><aside className="hidden lg:col-span-3 lg:block"><nav className="sticky top-24 text-sm" aria-label="Project overview contents"><p className="mb-4 font-medium">On this page</p><div className="flex flex-col gap-3 text-ink/55"><a href="#overview">Overview</a><a href="#focus">Current focus</a><a href="#approach">Approach</a></div></nav></aside>
      <div className="space-y-20 lg:col-span-7 lg:col-start-5">
        <section id="overview"><SectionTitle number="01">Project overview</SectionTitle><p className="text-xl leading-relaxed text-ink/70">{project.summary}</p></section>
        <section id="focus"><SectionTitle number="02">Current focus</SectionTitle><p className="mb-8 text-base leading-relaxed text-ink/65">This work is intentionally in progress. The current focus is on building a foundation that keeps the project coherent as it develops.</p><div className="overflow-hidden rounded-xl border border-ink/10 bg-[#fbfbf8]">{project.focus.map((item, itemIndex) => <div className="grid grid-cols-[48px_1fr] gap-4 border-b border-ink/10 p-5 last:border-b-0" key={item}><span className="text-sm text-moss">0{itemIndex + 1}</span><p className="text-base font-medium">{item}</p></div>)}</div></section>
        <section id="approach"><SectionTitle number="03">A deliberate foundation</SectionTitle><p className="text-xl leading-relaxed text-ink/70">The work emphasizes clear structure, reusable decisions, and room for thoughtful iteration. Details will be shared as the project evolves.</p></section>
      </div>
    </div></section>
